...
export default function TasksPage() {
  ...
  const fetchTasks = async () => {
    ...
    const { data, error } = await supabase.from("tasks").select("*, contacts(first_name, last_name)").eq("org_id", activeOrgId).order("due_date", { ascending: true });
    if (!error && data) setTasks(data);
  };

  useEffect(() => { fetchTasks(); }, [activeOrgId, userId]);
  ...
  const toggleTask = async (taskId: string, currentStatus: boolean) => {
    ...
    if (error) { toast.error("Failed to update task"); fetchTasks(); }
  };
  ...
  return (
    <AppLayout>
      <div className="p-4 md:p-8 max-w-3xl mx-auto">
        <SectionTitle eyebrow={currentWorkspace === "agency" ? "Agency Workspace" : "Clinic Workspace"} title="Tasks" action={<Button icon={Plus} onClick={() => setIsModalOpen(true)}>New Task</Button>} />
          ...
          return (
            <div key={group} className="mb-8">
              ...
              <div className="rounded-xl overflow-hidden border border-line bg-white shadow-card">
                {items.map((t) => (
                  <div key={t.id} className="flex items-center gap-3 px-4 py-3 border-b border-line last:border-0 hover:bg-paperDim/30 transition-colors group">
                    ...
                    <div className="flex-1">
                      <div className={`text-sm font-body font-medium ${t.is_completed ? "text-slate line-through" : "text-ink"}`}>{t.title}</div>
        ...
        {tasks.length === 0 && <div className="p-8 text-center text-slate font-body bg-white border border-line rounded-xl shadow-card">No tasks found.</div>}
      </div>
