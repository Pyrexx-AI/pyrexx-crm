...
export function Topbar() {
  ...
  return (
    <div className="flex items-center justify-between px-4 md:px-8 py-4 border-b border-line bg-paper z-30">
      <div className="flex items-center gap-3">
        <button onClick={() => setMobileMenuOpen(true)} className="md:hidden text-ink">
          <Menu size={20} />
        </button>
        <div className="text-sm text-slate font-body font-medium truncate max-w-[120px] sm:max-w-none">
          {getTitle()}
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-4 relative">
        ...
        {/* FIX: Bounded mobile dropdowns to max screen width */}
        <div className="relative" ref={notifRef}>
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-[calc(100vw-32px)] sm:w-72 max-w-[300px] bg-white rounded-xl shadow-xl border border-line overflow-hidden z-50">
